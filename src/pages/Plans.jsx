...
const Plans = () => {
  ...
  const quickActions = [
    { text: t('quickActions.comparePlans'), icon: Target, href: "#plans-section" },
    { text: t('quickActions.seeFeatures'), icon: TrendingUp, href: "#features" },
    { text: t('quickActions.startTrading'), icon: CreditCard, href: platformUrl },
  ];

  return (
    <DefaultLayout>
      <GradientIcon id="plans-icon-gradient" />
      <style>
        {`
          ...
          {quickActions.map((action) => (
            ...
            >
              <Button variant="outline" className="mb-2 bg-slate-800/50 border-white/20 hover:bg-slate-800 text-slate-200 hover:text-white transition-colors duration-300">
                <action.icon className="h-5 w-5 mr-2" style={{ stroke: "url(#plans-icon-gradient)" }} />
                {action.text}
              </Button>
            </motion.a>
          ))}
